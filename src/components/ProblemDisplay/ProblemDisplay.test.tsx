...
describe('ProblemDisplay', () => {
  it('renders multiplication problem correctly', () => {
    render(<ProblemDisplay factor1={4} factor2={6} />);
    const problem = screen.getByRole('region');
    expect(problem).toHaveTextContent('4 × 6 =');
  });

  it('applies custom className when provided', () => {
    render(<ProblemDisplay factor1={3} factor2={5} className="custom-class" />);
  ...
  it('throws error for invalid factor1', () => {
    expect(() => {
      render(<ProblemDisplay factor1={1} factor2={5} />);
    }).toThrow('Factors must be between 2 and 10');

    expect(() => {
      render(<ProblemDisplay factor1={11} factor2={5} />);
    }).toThrow('Factors must be between 2 and 10');
  });

  it('throws error for invalid factor2', () => {
    expect(() => {
      render(<ProblemDisplay factor1={5} factor2={1} />);
    }).toThrow('Factors must be between 2 and 10');

    expect(() => {
      render(<ProblemDisplay factor1={5} factor2={11} />);
    }).toThrow('Factors must be between 2 and 10');
  });

  it('meets accessibility requirements', () => {
    render(<ProblemDisplay factor1={7} factor2={8} />);
    const problem = screen.getByRole('region');
  ...
  it('maintains consistent layout with different number lengths', () => {
    const { rerender } = render(<ProblemDisplay factor1={2} factor2={3} />);
    const singleDigitProblem = screen.getByRole('region').textContent;
    
    rerender(<ProblemDisplay factor1={10} factor2={8} />);
