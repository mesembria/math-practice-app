 ...
 */
export async function getUserPerformanceTrends(userId: number, maxSessions: number = 20) {
  try {
    // Validate userId
    ...
    const sessions = await AppDataSource
      ...
      .select(['session.id', 'session.start_time', 'session.end_time'])
    ...
    // Query to get performance metrics for each session
